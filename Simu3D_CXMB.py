...
class Agent_sim(pyglet.window.Window):
    def __init__(self, name_saved,
        ...
        self.CX_FBn2PFL = np.zeros((len(self.CX_cards), len(self.CX_cards)*2))
        for ineux in range(self.CX_FBn2PFL.shape[0]):
            for ineuy in range(self.CX_FBn2PFL.shape[1]):
                ...
                elif (ineuy >= len(self.CX_cards)) & ((distNeu == 15) | (distNeu == 7)):
                    self.CX_FBn2PFL[ineux, ineuy] = 1.0
        ...
        #         if (ineuy < len(self.CX_cards)) & ((distNeu == -1) | (distNeu == 7)):
        #             self.CX_FBn2PFL[ineux, ineuy] = 1.0
        #         elif (ineuy >= len(self.CX_cards)) & ((distNeu == 9) | (distNeu == 1)):
        #             self.CX_FBn2PFL[ineux, ineuy] = 1.0
    ...
    def CX_update(self):
        ...
        self.CX_PFLR = np.asarray(self.CX_EPG[0, 0:8]).reshape((1, 8)) * self.CX_PFL3weight_L
        self.CX_PFLL = np.asarray(self.CX_EPG[0, 8:]).reshape((1, 8)) * self.CX_PFL3weight_R
    ...
    #### Color channel separation
        ...
        if self.init:
            ...
            self.PFL_activity.append(self.CX_PFL[0, :].tolist())
            self.EN_activity.append(np.concatenate((self.CX_hl, self.CX_hr)).tolist())
...
if __name__ == "__main__":
    ...
    name_folder = 'CXMB_EllipsoidBody_FB2.0_MBdoubleMemo_MB_MBpos_PosEPG2PFL_InvSteer_Simu500/'
    ...
    for iexp in range(NB_exp):
        ...
        Vin_K = 0.0#np.random.uniform(0.5, 1.0, 1)[0]
        ## Olfactory pathway
